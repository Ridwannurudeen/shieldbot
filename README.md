...
The core scan path refuses to call incomplete data safe and flags dangers supported by its checks. **This is not a claim that every interface blocks every unknown transaction.** Human interfaces can offer a proceed option; SDK callers must enforce the returned decision. Robinhood Chain tokens are compared with Robinhood's published list of official stock tokens: a listed contract is reported as official, a token whose ticker and name both copy an official one, claim Robinhood, or disguise it with look-alike characters, as an impostor, and a bare shared ticker or name, or a symbol in another issuer's convention (TSLAx named an xStock, TSLA.d, wTSLA, bTSLA named Backed), as a collision. That is a name check against Robinhood's list, not authentication of any other issuer.
