...
async def test_confirmation_registers_and_explicit_registration_is_idempotent(db):
    ...
    assert await db.register_guard_subject(4663, TOKEN.upper(), REGISTRY)
...
async def test_unqualified_confirmation_does_not_admit(db, chain_id, registry, observed_at):
    ...
    assert not await db.register_guard_subject(chain_id, TOKEN, REGISTRY)
...
async def test_cap_applies_to_admission_and_explicit_reentry(db, monkeypatch):
    ...
    assert not await db.register_guard_subject(4663, OTHER, REGISTRY)
    await db.unregister_guard_subject(4663, TOKEN)
    assert await db.register_guard_subject(4663, OTHER, REGISTRY)
    assert not await db.register_guard_subject(4663, TOKEN, REGISTRY)
...
async def test_concurrent_confirmations_across_connections_obey_cap(db, monkeypatch):
    ...
    try:
        await asyncio.gather(db.update_verdict_onchain(first, "confirmed", registry=REGISTRY),
                             peer.update_verdict_onchain(second, "confirmed", registry=REGISTRY))
        assert len(await db.get_guard_subjects(4663)) == 1
        await db.unregister_guard_subject(4663, TOKEN)
        await db.unregister_guard_subject(4663, OTHER)
        admitted = await asyncio.gather(db.register_guard_subject(4663, TOKEN, REGISTRY),
                                        peer.register_guard_subject(4663, OTHER, REGISTRY))
...
async def test_failed_measurement_and_late_old_confirmation_never_advance_or_regress_time(db):
    ...
    await db.update_verdict_onchain(incomplete, "confirmed", registry=REGISTRY)
