...
- `check_agent_reputation` is a block-rate heuristic over at most 1,000 local firewall records. An unregistered agent, or a registered agent with no history, returns a null trust score and `status: "unknown"`; a history that reaches the 1,000-record cap also stays `unknown`, because its count is a lower bound, and so does one with any record whose scan had incomplete coverage or that does not record its checks, with a null trust score and block rate.
- `scan_for_injection` checks a fixed regex list. Every result is `status: "unknown"`; `matched: false` means no listed pattern matched, not that the content is safe, and its `risk_level` is `UNKNOWN`. The returned depth label does not add a deeper analysis pass.
...
- `first`, the interim verdict. It is sent as soon as a floor reaches `BLOCK_RECOMMENDED` (an admin blacklist entry for the target, read locally before any provider answers; an analyzer's declared floor; a block-severity scam match), otherwise `FIRST_VERDICT_SECONDS` (3, `core/registry.py`) after the handler starts, and only while the scan is still running. The clock starts with the handler, not with the analyzers, because the handler's own lookups (token info, bytecode, `is_token`, verification) come first. A scan that finishes sooner sends no `first`, and neither does a signature request, which has nothing decoded to show. The target's own blacklist entry counts only when the target's own scan answers: a swap through a trusted router is answered from its path tokens, so a community entry on the router is left out of the first as it is of the final. The interim verdict always has `status: "unknown"` and is never `SAFE`. It is scored by the hard floors known so far and never by a partial weighted mean: the engine applies each floor to the final score with `max()`, so while the same evidence holds the interim band is never above the final one, while a partial mean could read HIGH_RISK and then be diluted to SAFE. With no floor it is `CAUTION` with `risk_score` 0 and `risk_display` "Unknown (analysis in progress)". It has the plain response's transaction fields, `coverage` and `notes` for the analyzers that have returned (the notes worded as the final's and never among its danger signals), `coverage_reasons.pending`, `pending_sources` (both keyed `token:analyzer` on a router swap, as the final's coverage is), `elapsed_ms`, `partial: true` and `final: false`, and no `evidence_hash`, `evidence_url` or `cached`.
...
Fields: `endpoint`; `source` (`scan`, or `cache` with `cached_scan_at`); `chain_id`; `target` (lower case; for a signature request, the first valid address among `to` and the typed data's verifying contract, spender and token, or the zero address when there is none, never the signer, so a raw eth_sign or personal_sign records its `to` or the zero address); `target_token` (name and symbol, when the scan resolved them); `classification`, `risk_score`, `risk_level` and `status` as the response gave them; `coverage`, `coverage_reasons` and `failed_sources`; `notes`, as the response gave them; `analyzers`, each analyzer's `status` (`ok`, `unknown`, `skipped` or `failed`), engine category `score`, `coverage`, `reason`, the coverage `fields` it declares (`answered` or `unknown`) and the `field_providers` it names (a router swap keys them `token:analyzer`). `failed` is recorded only for a whole analyzer (it raised or ran past the registry's deadline): the Unknown ledger counts provider outcomes for the whole process, not per scan, so a provider that failed and one that had no data both leave a field `unknown`, and `reason` says which; `policy_mode`; `observed_block` (the oldest block a sell simulation read, Robinhood Chain only, else null); `transaction`; `shieldbot_commit` (read from `.git` at startup, null when unreadable); and `scanned_at`, the time of the scan behind the verdict. A field the answering path does not report is null, never a default: the legacy fallback has no `risk_level`, `failed_sources` or `analyzers`, and `/api/scan` has no `analyzers`.
...
The browser also retains user overrides. In Balanced mode (the default) incomplete results and risk overlays offer "Proceed Anyway", while a Block Recommended verdict and an API error need Proceed (or Sign Anyway) held for 1.5 seconds. Strict mode removes Proceed and Sign Anyway from a Block Recommended verdict, from any incomplete result (Unknown, or High Risk or Caution with checks missing), from an API error and from a request whose structure or typed data could not be read; a complete High Risk or Caution result keeps Proceed in both modes. The chain-resolution path independently rejects unknown, unsupported or mismatched chains with no Proceed in either mode, and rejects a request whose chain changes during the check when the user proceeds; it does not remove those other overrides. Signature requests are analysed by the same `/api/firewall` endpoint (without the text of a message or the hash to sign). The popup's Wallet Health request uses the chain chosen in the side panel's chain selector (`selectedChainId` in extension storage, BNB Chain until one is chosen), the chain the side panel's Guardian tab also scans. The API answers a `personal_sign` without typed data `SAFE` with status `ok`, because the extension judges the message itself (a sign-in for another site, a raw hash): an API or SDK caller other than the extension gets no reading of the message content. Do not describe this extension as an unbypassable security boundary or claim that every incomplete check prevents signing.
...
- [ ] A scan answers even when a provider hangs. There is no 3 second target: the analyzers stop at the 25 s deadline (`RUN_ALL_DEADLINE_SECONDS`, `core/registry.py`), which starts after the bot's own token check, and an analyzer cut off there reads Unknown
...
- **STRICT and the score cache**: `/api/firewall` answers a repeat request for a target from its stored verdict for 5 minutes. The `X-Policy-Mode: STRICT` header skips that cache only on a request with a valid API key; any other STRICT request is answered from the cached facts judged in STRICT mode, so the header can no longer force a full uncached scan. STRICT blocks a fresh and a cached verdict on the same rule, a failed required check (`REQUIRED_COVERAGE` in `core/policy.py`): a scan's row keeps the policy engine's `failed_sources`, and a row stored without them (written before they were kept, or by a path with no policy engine) blocks on any Unknown. That does not make every request cacheable: transaction-specific requests (paying calls, approvals, typed-data signatures) and swaps through a trusted router always scan fresh by design, because their verdict depends on the transaction as well as the target. So does a target with a local blacklist entry (admin or community): its row keeps no scam matches, so an entry added since the row was written would otherwise be answered with the row. A community-listed target is therefore rescanned on every request, which costs provider calls each time; the IP and API key rate limits bound them. For those, the IP rate limiter above is the control on repeated fresh scans.
