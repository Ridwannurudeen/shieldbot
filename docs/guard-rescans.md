...
23-request budget as launch scans and obey the same breaker. Failed, incomplete or reused
...
**A lost interval is structural.** In [agent/hunter.py](../agent/hunter.py#L222), `rescan_guard_subject` accepts a complete
scan whose publication status is `pending` (lines 222–225) and persists its `observed_at` through
`update_guard_subject_measurement` (lines 234–237). This advances `last_observed_at` before the drain
confirms publication. The next due check uses that observation clock (lines 176–179), not the last
...
validation can drop stale or superseded rows before broadcast ([services/verdict_publisher.py](../services/verdict_publisher.py#L736),
`_drop_ineligible`, lines 736–762). This explains why a lost interval can exceed the 600-second demo window.

**Publication precedes completeness validation.** The [hunter calls `_publish_verdict` at line 199](../agent/hunter.py#L199),
then computes `complete` at lines 202–208. A scan overrun returning incomplete evidence can therefore
...
These source line numbers were checked on 2026-09-26 at `f2861bb`. See the
