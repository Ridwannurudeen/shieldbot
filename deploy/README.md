...
5. requires `/api/health` to report `status: ok` with exactly the chains in the deployed
   `utils/chain_info.py`, then `/api/ready` to answer 200 within about a minute (the database and at least one
   chain's RPC answering), and `/api/stats` to answer. An outage of every RPC, or of the first ones asked, while
   the deploy runs therefore rolls it back too; run it again once they answer
