...
- `inject.js` runs in the page's own JavaScript world at document_start, in every frame. It wraps
  ...
  `request` on every prototype of the provider that defines one (up to `Object.prototype`), so
  calling a prototype's method on the provider is checked too. It takes every built-in it uses at
  startup, before any page script runs.
...
- `send` and `sendAsync`, the older provider methods, are replaced the same way, on the provider and
  ...
  promise for `send(method, params)`, or by throwing for the synchronous form. Other calls reach the
  wallet with their own `this` and arguments, a payload object being handed on as the copy its
  methods were read from.
...
- It never passes a request it checks to the wallet without showing it to the user first. If no
  ...
  element, calling `document.open()` or moving it into another document), the request is rejected.
  Strict mode leaves no Proceed or Sign Anyway button when the analysis failed, the verdict is
  Unknown or Block Recommended, the structure is unknown, or typed data cannot be read.
...
- Replacing a prototype's `request` cannot tell the provider from other objects: another object of
  ...
  provider when it calls that prototype's `request`. And a page may have taken such a prototype's
  own function before the provider was wrapped (for example before a late EIP-6963 announcement);
  that copy reaches the wallet unchecked.
- A page can hide or cover the warning, or lay something over it to trick a click (clickjacking).
  ...
  the warning appears, and do nothing while IntersectionObserver v2 reports the dialog as not
  visible (off screen, covered, or made transparent, filtered or transformed). Users should always
  read the wallet's own confirmation.
