...
| `shieldbot://agent/{agent_id}/health` | `resources/templates/list` | Policy and the 20 latest firewall verdicts of a registered agent. An agent registered with an API key is readable only by that key, and to any other key reads exactly as an unregistered one; an agent registered before keys were recorded is readable by any key, as on the REST agent routes. |
